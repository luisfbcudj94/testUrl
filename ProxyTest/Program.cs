...
foreach (var url in urls)
{
    ...
    session.DevToolsEventReceived += (sender, e) =>
    {
        ...
        if (e.EventName == "requestWillBeSentExtraInfo")
        {
            ...
            {
                stopSession = true;
                DataPreprocessing(dataToExcelRequest, dataToExcelResponse);
                driver.Quit();
...
static void DataPreprocessing(Dictionary<string, List<JObject>> dataToExcelRequest, Dictionary<string, List<JObject>>  dataToExcelResponse)
{
...
static async void WriteDictionaryToCsv(Dictionary<string, List<JObject>> dictionary, string filePath)
{
